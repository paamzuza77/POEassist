...
2. **(ต้องทำต่อ)** เพิ่ม `.github/workflows/deploy-pages.yml`: checkout → setup-node → `npm ci` → `npm run build` →
   `actions/upload-pages-artifact` (path `dist`) → `actions/deploy-pages` (permissions: pages write, id-token write). js/ea.js
   commit อยู่ใน repo แล้ว → checkout มีครบ; data/*.json สดจาก workflow เดิม.
3. **(ผู้ใช้ต้องกดเอง)** สลับ GitHub → Settings → Pages → Source เป็น **"GitHub Actions"** (จาก "Deploy from a branch").
   *ก่อนสลับ:* deploy เดิม (เสิร์ฟ root index.html) ยังทำงาน = **ไม่มีดาวน์ไทม์**. หลังสลับ: push main → build+deploy อัตโนมัติ.
4. ✅ data/*.json copy เข้า dist แล้ว (ข้อ 1) — workflow `update-market-radar.yml` ยัง commit data ทุกชม.; deploy job checkout
   จะได้ data สด. (ระวัง loop: `update-market-radar.yml` มี paths-ignore; deploy job ควร trigger เฉพาะที่จำเป็น.)
