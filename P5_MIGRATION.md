...
- **ที่เหลือ (พึ่ง mutable global — ยังไม่ย้าย):** forge scoring `computeForgeReadiness`/`computeForgeMissing`/`buildForgeRecos`
  (พึ่ง `forgeTargets`/`EQUIP_SLOTS`/`slotHasAnyData` — CAP/PENALTY ยกแล้ว), shop `rowEstPrice`/`shopBuyNextCandidates`
  (พึ่ง `shopRows`).
  **จะย้ายได้เมื่อ:** ส่ง forgeTargets/shopRows เข้าเป็น argument (เปลี่ยน signature หลาย call site — วิธีเดียวกับ radar 0.71)
  หรือยก mutable state ขึ้น module. **constants ยกครบแล้ว (0.70), radar mutable-state ย้ายแล้ว (0.71)** — เหลือ forge + shop.
