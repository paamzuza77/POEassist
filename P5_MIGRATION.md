...
- **ผู้สมัครถัดไป (pure logic ของแท็บ):** `cmdkFuzzyScore` (palette), `radarFmtValue`/`priceSparkline` (radar),
  `parseIso`/`formatDurationParts`-family (มี format แล้ว), forge scoring (`computeForgeReadiness`/`computeForgeMissing`
  — เช็คว่าพึ่ง global ไหมก่อน), shop `rowEstPrice`/`shopBuyNextCandidates` (พึ่ง global บางส่วน — ระวัง).
  **กติกา:** ย้ายเฉพาะ pure (input→output ไม่แตะ DOM/global) ก่อน; ที่พึ่ง state รอจนกว่า state จะขึ้น bridge.
