...
- **ขั้นถัดไป:** `js/ux-foundation.js` → `src/lib/ux-foundation.ts` (toasts/states/micro — DOM-heavy กว่า, มี
  ~15 global: showToast family, uxEmptyState, uxSkeleton, uxFlash, uxBusy ฯลฯ). ทำแบบเดียวกัน: พอร์ต+type →
  re-export → build → ลบ script+ไฟล์เดิม → bind ทั้งชุดจาก window.EA. **ระวัง:** `css/asset-icons.css`,
  `css/ux-foundation.css` ยัง `<link>` แยกอยู่ (ไม่ย้าย — เป็น CSS). `KWC_ASSET_IDS` ยัง inline (ไม่เกี่ยว).
