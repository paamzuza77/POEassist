...
- **ที่เหลือ (พึ่ง mutable global — ยังไม่ย้าย):** forge scoring `computeForgeReadiness`/`computeForgeMissing`/`buildForgeRecos`
  (พึ่ง `forgeTargets`/`EQUIP_SLOTS`/`slotHasAnyData` — CAP/PENALTY ยกแล้ว; ต้องพา `slotHasAnyData` ไปด้วย).
  **จะย้ายได้เมื่อ:** ส่ง forgeTargets (+ slotHasAnyData/EQUIP_SLOTS) เข้าเป็น argument (วิธีเดียวกับ radar 0.71/shop 0.72)
  หรือยก mutable state ขึ้น module. **constants (0.70) + radar (0.71) + shop (0.72) ย้ายแล้ว** — เหลือ forge เป็นก้อนสุดท้าย.
