...
- **ที่ข้ามเพราะพึ่ง global runtime state ที่ mutable (ยังไม่ย้าย):** `radarSnapshotAgeHours`/`radarConfidenceInfo` (พึ่ง
  `radarData` — reassign ตอนโหลด; RADAR_FRESH_HOURS ยกขึ้น constants แล้ว), forge scoring `computeForgeReadiness`/
  `computeForgeMissing`/`buildForgeRecos` (พึ่ง `forgeTargets`/`EQUIP_SLOTS`/`slotHasAnyData` — CAP/PENALTY ยกแล้ว),
  shop `rowEstPrice`/`shopBuyNextCandidates` (พึ่ง `shopRows`).
  **จะย้ายได้เมื่อ:** ส่ง radarData/forgeTargets/shopRows เข้าเป็น argument (เปลี่ยน signature หลาย call site) หรือยก mutable
  state ขึ้น module. **constants ยกครบแล้ว (patch 0.70)** — เหลือแค่ mutable state เป็นงานก้อนถัดไป.
