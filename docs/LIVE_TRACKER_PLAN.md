...
### 4.1 สิ่งที่พบจากการ probe จริง (2026-07-23, ลีค Runes of Aldur)

- Endpoint เดิมใช้ต่อได้: `index-state` → `version` → `/poe2/api/economy/exchange/{version}/overview?league={display}&type={Type}`
- **Script ปัจจุบันดึง 11 หมวด:** Currency, Fragments, Abyss, Runes, Ritual, Delirium, Breach, SoulCores,
  Essences, Expedition, Verisium
- **Probe เจอหมวดที่มีข้อมูลจริงเพิ่ม ≥11 หมวด:** `Catalysts`, `Waystones`, `Talismans`, `Gems`, `Omens`,
  `Distilled`, `Artifacts`, `Logbooks`, `LiquidEmotions`, `Tablets`, `LineageSupportGems` (78 items!)
- **Sidebar หน้าเว็บมีชื่อที่ probe แล้วชื่อ type ไม่ตรง:** "Uncut Gems", "Idols", "Abyssal Bones" (ชื่อ UI ≠
  ชื่อ type param; `Abyss` ใช้ได้ ส่วน UncutGems/Idol คืนโครงว่าง) → **ต้องทำ probe อัตโนมัติใน script**
  อย่า hardcode อย่างเดียว
...
|---|---|---|---|
| A1. ขยาย poe.ninja ครบทุกหมวด → `market-prices.json` | A | S-M | ไม่มี — ทำก่อนเพราะ A2 ใช้ |
| A2. Live Farm Session (เว็บล้วน) ใน Farm Planner | A | M-L | ใช้ A1 |
| A3. price lookup เปลี่ยน source เป็น market-prices | A | S | หลัง A1 |
