...
- Sidebar nav buttons: still one `<button class="nav-item" id="tabBtnX">` per tab (was `class="tab-btn"`), grouped inside `<div class="nav-group">` blocks under a `<div class="nav-group-label">` (search `class="nav-group"` in `index.html`) — currently **Main** (Exile Hub/Market Radar/Divine Market), **Planning** (Farm Planner/Gear Planner), **Tools** (Resistance Checker); Patch Notes lives in `.sidebar-footer` as a **System** group of one. **Visible names renamed in patch 0.20** (internal ids unchanged): Atlas Planner→**Farm Planner** (`farm`), Shopping List→**Gear Planner** (`shop`), Gear Checker→**Resistance Checker** (`forge`). Rename touch-points: the nav-button label text, `TAB_TITLES` (topbar title), and `MENU_ICON_LABELS` (icon fallback) — all three carry the visible string; ids/functions/CSS classes/storage keys keep the old names. **Atlas V.2 (`farmv2`) tab was removed in 0.20** — no nav button/page/JS/CSS remains; don't re-add it.
...
2026-07-12 (patch 0.20 — tab renames [Atlas Planner→Farm Planner / Shopping List→Gear Planner / Gear Checker→Resistance Checker, internal ids `farm`/`shop`/`forge` unchanged], removed the Atlas Farm V.2 tab entirely, Farm Planner duration switched to H/M/S with `farmFmtHMS` display, and left-list projected-net `.fl-profit` chip). Prior: patch 0.19 — Run Tracker analytics + view-state key `poe2FarmPlanner.view.v1`.
