...
| Shopping List (`shop`) | `index.html` — markup `#pageShop` (~2100), JS `==== Build Shopping List ====` (~3145) | `newShopRow()`, `updateShopSummary()`, `renderShopBuyNext()`, `sortShopRowsForDisplay()`, `buildLinkCell()`, `renderShopTable()` | `localStorage` (`poe2ShopList.rows.v1`) — row shape: `{checked, name, link, type, option, min, max, bought, dealLink, dealPrice}` (`name` added 2026-07-12, additive; old rows default `''`) | Keep link fields as chips (`buildLinkCell`), never raw URLs. "ต้องใช้อีก" (remaining) sums prices of **unbought rows only** (not planned − spent). Quick filters + sort select are in-memory display-only — never `sort()` `shopRows` itself; row actions resolve the live index via `shopRows.indexOf(row)`. Buy-next picks unbought rows with a price: checked first, then cheapest (`rowEstPrice`: deal > min > max) | Don't touch Atlas Planner/Gear Checker |
