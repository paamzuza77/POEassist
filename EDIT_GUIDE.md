...
- **Settings placeholder (2026-07-12):** `#settingsBtn` in the `.sidebar-footer` System group, **above** `#patchNotesBtn`; `initSettings` (search it) builds a RawBlock modal reusing `.patch-overlay`/`.patch-panel` (+ `.settings-panel`) with a coming-soon state — no real settings backend. To build real settings later, replace the modal body; keep it above Patch Notes.
