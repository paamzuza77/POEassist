...
| Atlas Planner (`farm`) | `index.html` — markup `#pageFarm` (~1877), JS `==== Atlas Farm Planner ====` | `newFarmCard()`, `buildFarmCard()`, `renderFarmGrid()`, `FARM_GOALS`/`FARM_MECHANICS`/`FARM_STATUSES` | `localStorage` only (`poe2FarmPlanner.cards.v1`) | Add dropdown options via the arrays; card layout is in `buildFarmCard()`. Patch 0.16: filter chips show live counts (`updateFarmFilterCounts()` — hooked into `persistFarmCards()`, so any card mutation refreshes them); card foot has a `.farm-checked-age` last-checked age label (⚠ `.divine-warn` at ≥14 days) | Don't touch Market Radar scoring |
...
2026-07-12 (patch 0.16 — app-wide QoL pass: Ends-in countdown, radar TOP PICK, Divine reorder+prefill, farm filter counts + age warnings, gear preset dots + cap counter, shop quick-filter counts)
