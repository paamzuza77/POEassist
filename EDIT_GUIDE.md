...
- **Settings (2026-07-12; functional since patch 0.13):** `#settingsBtn` in the `.sidebar-footer` System group, **above** `#patchNotesBtn`; `initSettings` (search it) builds a modal reusing `.patch-overlay`/`.patch-panel` (+ `.settings-panel`). It now holds real controls: **Theme** (RawBlock / Trust Blue Pay) and **Mode** (Light / Dark) segmented buttons (`#appearanceThemeSeg` / `#appearanceModeSeg`, class `.appearance-seg`). These call `setAppTheme()` / `setAppColorMode()` (the shared appearance module — search `THEME_KEY`), which persist and re-`syncAppearanceControls()`. Other prefs (Divine rate, rows, table sizing, sidebar collapse, gear presets) still autosave outside Settings. To add more settings, extend the modal body; keep it above Patch Notes.
...
**Theme switcher + dark/light mode (patch 0.13):** appearance is controlled by two attributes on `<html>` — `data-theme` (`rawblock` | `trust-blue-pay`) and `data-mode` (`light` | `dark`) — set pre-paint by a small `<head>` script and managed by the appearance module in the main script (search `THEME_KEY`, `MODE_KEY`, `loadThemePreference`, `applyThemePreference`, `setAppTheme`, `setAppColorMode`, `syncAppearanceControls`). Persisted in `poeAssist.theme.v1` / `poeAssist.colorMode.v1` (additive, tolerant). Defaults: RawBlock + light.
  ...
  - **Trust Blue Pay** (navy/clean-white "financial dashboard" per `trust-blue-pay-DESIGN.md`) also swaps fonts to Inter / JetBrains Mono and **softens structure** (1px thinner borders, ~10px radius, subtle navy shadows) via selectors scoped as `html[data-theme="trust-blue-pay"] .<class>` — those rules set border-width/radius/shadow only; colors still come from the tokens. If you add a new prominent surface/card, add its class to that softening list so it matches.
...
2026-07-12 (patch 0.13 — theme switcher + dark/light mode + Shopping List Type-column cleanup)
