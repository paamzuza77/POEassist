...
2026-07-12 (patch 0.18 — Atlas Planner master/detail redesign: left list + right detail, additive run-economics fields + per-card Run Tracker on `poe2FarmPlanner.cards.v1` (key unchanged), shared numpad selector `NUMPAD_INPUT_SELECTOR` now covers `#pageFarm` too)
