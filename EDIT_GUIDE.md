...
| Market Radar (`radar`) | `index.html` — markup `#pageRadar` (~1837), JS `==== Market Farm Radar ====` (~3980 area) | `radarItemScore()`, `buildRadarRecos()`, `renderRadar()` | `data/market-radar.json` ← `scripts/update-market-radar.mjs` | Scoring lives only in `radarItemScore`/`buildRadarRecos` — never rename the score to "drop chance" | Don't touch Atlas Planner card code |
...
- **Edit Market Radar scoring** — search: `function radarItemScore`. Files: `index.html` (frontend recompute) and/or `scripts/update-market-radar.mjs` (source data). Avoid: Atlas Planner card code. Verify: `node scripts/update-market-radar.mjs` runs clean; reload radar tab.
